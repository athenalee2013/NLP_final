...
#embeddingsPath = 'word_embedding/glove.6B.50d.txt'
embeddingsPath = 'word_embedding/wiki_extvec'
#embeddingsPath = 'word_embedding/glove.6B.300d.txt'
...
minDistance = -30
maxDistance = 30
for dis in range(minDistance,maxDistance+1):#xrange
...
def createMatrices(file, word2Idx, maxSentenceLen=100):
    ...
    for line in open(file):
        ...
        for idx in range(0, min(maxSentenceLen, len(tokens))):#xrange
...
for fileIdx in range(len(files)):#xrange
    file = files[fileIdx]
    for line in open(file):
        splits = line.strip().split('\t')
